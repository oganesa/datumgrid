    ...
    <div ref={containerRef} className="relative">
      ...
      <div className="relative">
        <input
          ...
          autoComplete="off"
          className="w-full rounded border border-[#E5EAF2] p-2 pr-7 text-sm outline-none focus:border-[#4A90E2]"
        />
        {inputValue && (
          <button
            type="button"
      ...
      {showDropdown && filtered.length > 0 && (
        <ul className="absolute z-20 mt-1 max-h-48 w-full overflow-y-auto rounded border border-[#E5EAF2] bg-white shadow-lg">
          {filtered.map((o) => (
            <li key={o._id}>
              <button
                type="button"
      ...
      {showDropdown && filtered.length === 0 && (
        <div className="absolute z-20 mt-1 w-full rounded border border-[#E5EAF2] bg-white px-3 py-2 text-sm text-gray-500 shadow-lg">
          No matching assets
        </div>
      )}
    </div>
