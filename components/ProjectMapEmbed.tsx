...
export default function ProjectMapEmbed({ project }: Props) {
  ...
  if (!q.trim()) {
    return (
      <div className="flex h-56 items-center justify-center rounded-md border border-dashed border-[#E5EAF2] bg-white text-center text-xs text-gray-500">
  ...
  if (!apiKey) {
    return (
      <div className="space-y-2 rounded-md border border-[#E5EAF2] bg-white p-4 text-xs text-gray-600">
        <p>{q}</p>
        <a
          href={searchHref}
          target="_blank"
          rel="noreferrer"
          className="inline-block text-[#4A90E2] hover:underline"
        >
          Open in Google Maps
        </a>
  ...
  return (
    <div className="overflow-hidden rounded-md border border-[#E5EAF2] bg-white shadow-sm">
      <iframe
